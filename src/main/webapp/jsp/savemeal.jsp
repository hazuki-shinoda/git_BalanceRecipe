...
<c:if test="${not empty ERROR_MSG}">
    <p class="success-msg"><%= Util.replaceEscapeChar((String)request.getAttribute("ERROR_MSG")) %></p>
</c:if>
  <h2>食事入力フォーム</h2>                                    
  <form action="SaveMealSurvey" method="post">                      
    <p>
    名前： <%= Util.replaceEscapeChar((String)request.getAttribute("USER_NAME_RAW")) %>
    <input type="hidden" name="NAME" value="<%= Util.replaceEscapeChar((String)request.getAttribute("USER_NAME_RAW")) %>">
	</p>
...
<nav>
    <ul style="list-style: none; padding: 0;">
        <li> <a href="Home">🏠 ホーム（今日の合計）へ</a>		</li>
        <li> <a href="ShowAllSurvey">📅 過去の記録一覧へ</a>	</li>
